<nav class="px-2 space-y-1">
    ...
    <div x-data="{ open: {{ request()->is('frontend*') ? 'true' : 'false' }} }" x-init="open = {{ request()->is('frontend*') ? 'true' : 'false' }}" class="mb-2">
        <button @click="open = !open"
            class="group mb-2 flex justify-between items-center w-full px-2 py-2 text-sm font-medium rounded-md 
        {{ request()->is('frontend*') ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-blue-100 hover:text-blue-700' }}">
            <div class="flex items-center">
                <x-heroicon-o-eye
                    class="mr-3 h-6 w-6 {{ request()->is('frontend*') ? 'text-blue-700' : 'text-[#25304e]' }}" />
                <span class="sidebar-item-text">Frontend</span>
            </div>
            <svg class="h-5 w-5 {{ request()->is('frontend*') ? 'text-blue-700' : 'text-gray-400' }}"
                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd"
                    d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                    clip-rule="evenodd" />
        ...
        <div x-show="open" class="pl-8 space-y-1">
          ...
          @if (request()->is('client*')) bg-[#112695] text-white @else text-gray-600 hover:bg-blue-100 hover:text-blue-700 @endif">
            ...
            <a href="{{ url('contact') }}"
                class="flex items-center px-2 py-2 text-sm font-medium rounded-md transition-all
       @if (request()->is('contact*')) bg-[#112695] text-white @else text-gray-600 hover:bg-blue-100 hover:text-blue-700 @endif">

                <x-heroicon-o-user-circle
                    class="w-5 h-5 mr-3 flex-shrink-0
           @if (request()->is('contact*')) text-white @else text-[25304e] @endif" />
